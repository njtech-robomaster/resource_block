...
/*角点判断函数*/std::vector<std::vector<cv::Point>> EDGE_JUI(std::vector<std::vector<cv::Point>> EDGES_BE, std::vector<std::vector<cv::Point>> EDGES_UN);
...
int main() {
    ...
    while (true) {
        ...
        EDGES = EDGE_JUI(EDGES_BE, EDGES_UN);
        ...
        if (ReCENTRE.size() == 4) {
            ...
            float DISTANS = PIX2MM(src_cp, ReCENTRE, RECENTRE);
        }
...
std::vector<std::vector<cv::Point>> processs(cv::VideoCapture cap, cv::Mat& src_out, std::vector<double>& ANGLE) {
    ...
    src_L = src_3C.at(2);
    cv::threshold(src_L, src_bi, 160, 255, cv::THRESH_BINARY);
    ...
    for (int i = 0; i < contours.size(); i++) {
        ...
        cv::approxPolyDP(contours[i], conPoly[i], 0.025 * peri, true);
